...
import apiClient from "@/api/client"; // 引入 apiClient
...
const fetchDataForColumnChart = async () => {
  try {
    const response = await apiClient.get("/houses/stats/column");
    // 响应拦截器已解包，response.data 直接是 {community_list, num_list}
    ...
    // 让 lineDataForSeries 是 seriesValueData 的一个简单变换
