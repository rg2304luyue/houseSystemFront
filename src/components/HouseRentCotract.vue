<template>
  ...
  >
    ...
    <v-card-text>
      ...
      <p>甲方将位于 {{ property.region }} 的房屋出租给乙方使用。</p>
      <p>房屋标题：{{ property.title }}</p>
      <p>房屋内现有设施设备情况（可另附清单）：</p>

      <p><strong>二、房屋用途</strong></p>
      <p>该房屋用途为 {{ purpose || property.purpose || '______' }}。未经甲方书面同意，乙方不得擅自改变房屋用途。</p>

      <p>乙方保证在租赁期间内，按照约定用途合理使用房屋，不得利用房屋从事违法违规活动。</p>

      <p><strong>三、租赁期限</strong></p>
      
      <p>租赁期限自 {{ formattedStartDate || property.startDate || '______' }} 起至 {{ formattedEndDate || property.endDate || '______' }} 止。</p>
      ...
      <p>该房屋月租金为人民币{{ rentValue || property.rentValue || '______' }}元整（大写：{{ formatCurrency(rentValue || property.rentValue) || '______' }}元整）。</p>
...
// 表单数据
const formData = ref({
  rentValue: props.property.rentValue,
  purpose: props.property.purpose,
  startDate: props.property.startDate,
  endDate: props.property.endDate,
  landlordName: props.property.landlord_username,
  landlordPhone: props.property.landlordPhone,
  tenantName: username,
  tenantId: '',
  tenantPhone: ''
})
...
const submitContract = async () => {
  try {
    ...
    // 准备完整的数据对象
    const payload = {
      rentValue: rentValue.value || props.property.rentValue,
      purpose: purpose.value || props.property.purpose,
      startDate: startDate.value || props.property.startDate,
      endDate: endDate.value || props.property.endDate,
      landlordName: props.property.landlord_username,
      landlordPhone: props.property.landlordPhone,
      tenantName: username,
      formattedRent: formatCurrency(rentValue.value || props.property.rentValue),
...
// 从 props 获取数据
const landlordName = ref(props.property.landlord_username)
const landlordPhone = ref(props.property.landlordPhone)
const rentValue = ref(props.property.rentValue)

// 获取当前日期
const currentDate = new Date().toISOString().split('T')[0];

const startDate = ref(props.property.startDate);
const endDate = ref(props.property.endDate);
const showStartPicker = ref(false);
const showEndPicker = ref(false);
const purpose = ref(props.property.purpose);
